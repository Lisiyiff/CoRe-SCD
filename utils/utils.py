...
def split_train_val(dataset, val_percent=0.05):
    ...
    return {'train': dataset[:-n], 'val1': dataset[-n:]}
...
# ========== 多指标评价 ==========
def cal_kappa_v2(hist):
    po = np.diag(hist).sum() / (hist.sum() + 1e-10)
    pe = np.matmul(hist.sum(1), hist.sum(0).T) / ((hist.sum() + 1e-10) ** 2)
    return (po - pe) / (1 - pe + 1e-10)

def SCDD_eval_all_v2(preds, labels, num_class):
    hist = np.zeros((num_class, num_class))
    for pred, label in zip(preds, labels):
        infer_array = np.array(pred)
        label_array = np.array(label)
    ...
    hist_n0 = hist.copy()
    hist_n0[0][0] = 0
    kappa = cal_kappa_v2(hist)
    kappa_n0 = cal_kappa_v2(hist_n0)
    iu = np.zeros(2)
    for k in range(2):
        denom = c2hist.sum(1)[k] + c2hist.sum(0)[k] - c2hist[k, k]
        iu[k] = c2hist[k, k] / (denom + 1e-10)
    IoU_fg = iu[1]
    IoU_mean = (iu[0] + iu[1]) / 2 if not np.isnan(iu).any() else 0.0
    SeK = (kappa_n0 * np.exp(IoU_fg)) / np.e
    pixel_sum = hist.sum()
    change_pred_sum = pixel_sum - hist.sum(1)[0]
    change_label_sum = pixel_sum - hist.sum(0)[0]
    SC_TP = np.diag(hist[1:, 1:]).sum()
    Precision = SC_TP / (change_pred_sum + 1e-10)
    Recall = SC_TP / (change_label_sum + 1e-10)
    OA = np.trace(hist) / (pixel_sum + 1e-10)
    if Precision > 0 and Recall > 0:
        Fscd = stats.hmean([Precision, Recall])
    else:
        Fscd = 0.0
    return {
        'Fscd': Fscd,
