...
class BinaryDiceLoss(nn.Module):
    ...
    def forward(self, pred_prob, target_bin, valid_mask=None):
        """
        pred_prob: (N,1,H,W) 概率[0,1]
        target_bin: (N,1,H,W) {0,1}
        valid_mask: (N,1,H,W) {0,1}，像素级有效掩膜，可为 None
        """
        if valid_mask is None:
            valid_mask = torch.ones_like(target_bin)
        pred = pred_prob * valid_mask
        tgt  = target_bin * valid_mask

        intersection = (pred * tgt).sum(dim=(2,3))
        denom = pred.sum(dim=(2,3)) + tgt.sum(dim=(2,3))
...
class MultiClassDiceLoss(nn.Module):
    """
    多类 Dice，支持：
      - from_logits: 传入 logits 自动 softmax
      - ignore_label: 像素级忽略值（如 255）；会构造 valid_mask
      - class_weight: (C,) 类别权重
    """
    def __init__(self, num_classes, from_logits=True, ignore_label=None, class_weight=None, smooth=1.0):
        super().__init__()
        ...
        if class_weight is not None:
            self.class_weight = torch.as_tensor(class_weight, dtype=torch.float32)
        self.bin_dice = BinaryDiceLoss(smooth=smooth, reduction='mean')

    def forward(self, predict, target):
        """
        predict: (N,C,H,W) logits 或 概率
        target : (N,H,W) Long 索引 或 (N,C,H,W) one-hot
        """
        N, C, H, W = predict.shape
        assert C == self.num_classes, "num_classes 不匹配"

        # 1) 概率
        prob = F.softmax(predict, dim=1) if self.from_logits else predict

        # 2) 目标 one-hot & 有效掩膜
        if target.dim() == 3:
            # 像素级 ignore
            if self.ignore_label is not None:
                valid_mask = (target != self.ignore_label).unsqueeze(1).float()  # (N,1,H,W)
                tgt = target.clone()
                tgt[~valid_mask.squeeze(1).bool()] = 0  # 占位
            else:
                valid_mask = torch.ones((N,1,H,W), device=target.device, dtype=torch.float32)
                tgt = target
            target_1h = F.one_hot(tgt.clamp(min=0), num_classes=C).permute(0,3,1,2).float()  # (N,C,H,W)
            # 把 ignore 像素置零
        ...
        weights = []
        for i in range(C):
            # 若有 channel 级忽略（极少见），可在此跳过
            pred_i = prob[:, i:i+1, ...]
            tgt_i  = target_1h[:, i:i+1, ...]
            loss_i = self.bin_dice(pred_i, tgt_i, valid_mask)
            if self.class_weight is not None:
                w = self.class_weight[i].to(predict.device)
            else:
                w = torch.tensor(1.0, device=predict.device)
            losses.append(loss_i * w)
            weights.append(w)

        loss = torch.stack(losses).sum() / (torch.stack(weights).sum() + 1e-6)
        return loss
...
class BinaryDiceLoss(nn.Module):
    """
    二类 Dice（输入为概率图）
    pred_prob, target_bin: (N,1,H,W), in [0,1]
    可选 valid_mask: (N,1,H,W)
    """
    def __init__(self, smooth=1.0, reduction='mean'):
        super().__init__()
        self.smooth = smooth
